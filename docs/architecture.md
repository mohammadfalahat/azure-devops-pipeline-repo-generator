...
This document describes version 0.1.32 from the implementation in
...
All provisioning calls are made directly from the user's browser to the same
Azure DevOps collection that hosts the extension. The security principal is the
...
grant those resource-area permissions.
...
| `index.html` / `ui.js` | Dialog or `pipeline-generator-hub` host iframe | Reads host configuration/navigation state, obtains the current-user host token, hydrates the form, performs five provisioning steps, displays errors, and navigates the host to the Pipeline |
...
| Service | Lowercase source repository name; remains user-editable |
| Environment | `demo`, then inferred from source branch when possible |
...
| Komodo server | Derived from selected environment |
...
Changing the environment also changes the Komodo server to the configured
environment mapping. A user can then choose another available target manually.
...
```text
<sanitized-project>-<sanitized-source-repository>-<sanitized-source-branch>.yml
```
...
/ridesharing-ridesharing_backend-feature-definezones.yml
```
...
Release:  <pipelineName><releaseConfig.nameSuffix>
```
...
`/ridesharing-ridesharing_backend-feature-definezones.yml` maps to Pipeline name
`ridesharing-ridesharing_backend-feature-definezones.yml`.

The default Release suffix is `_Release`, so the matching Release name ends in
`.yml_Release`. Because source branch is part of the filename, different source
branches receive different Pipeline/Release identities.
...
### Step 1: ensure generated repository

The UI lists project repositories using Git API 6.0 and compares exact names.
If the target repository is missing, it creates it in the current project.
...
The UI searches Pipelines by exact name.
...
- If the name is absent, it searches Build Definitions by generated repository
  and exact YAML path. This finds a legacy same-file Pipeline with an old name;
  if the old bug created more than one match, the lowest/oldest definition ID
  is selected deterministically and unrelated duplicates are left untouched.
...
| --- | --- | --- |
| Generated repository | Exact repository name | Reuse |
...
| Pipeline | Exact filename name, then repository + YAML path | Reuse or GET-modify-PUT through Build Definitions |
...
Release definition result, waits 700 milliseconds, and asks the Azure DevOps
navigation service to navigate the parent host to:

```text
<collection>/<project>/_build?definitionId=<pipeline-id>
```

It never redirects only the dialog iframe or opens the YAML file as the final
success behavior. Top-level navigation is the fallback when the navigation
...
- Shared template repository, endpoint, variable group, template filename,
  Pipeline folder, and API versions are constants in `dist/ui.js`.
...
- Pipeline and Release names intentionally include the branch-specific YAML
  filename.
...
- Pipeline migration depends on Build Definitions list filtering by repository
  and YAML filename; Release migration filters expanded artifacts by type
